...
| 12 | Audit existing shell bricks against the rubric | **In progress (2026-06-13)** — ✅ **`ConfirmModal`** hardened (focus trap + restore, unique ids, `aria-describedby`). ✅ **`AddPersonByName`** (18 consumers, rubric pt 7): input now has an accessible name (`aria-label` ← new `label` prop, defaults to `placeholder`; was placeholder-only); duplicate-name error is announced (`role="alert"`) and linked via `aria-describedby` + `aria-invalid`. No API change (all props backward-compatible). **Still to audit:** `ClaimProfile`, `SwitchProfile`, field bricks (`AvatarField`/`ImageField`/`AccentColorField`/`EmojiPicker`). |

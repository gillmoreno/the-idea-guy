...
| 12 | Audit existing shell bricks against the rubric | **In progress (2026-06-13)** — ✅ **`ConfirmModal`** (focus trap + restore, unique ids, `aria-describedby`). ✅ **`AddPersonByName`** (accessible name, announced error). ✅ **`SwitchProfile`** (audited — already clean: one labelled `<button>`). ✅ **`ClaimProfile`** (19 consumers): **was hand-rolling an avatar circle and a raw `<div className="empty">`** — both missed migrations (the EmptyState sweep skipped `shell/`); now uses kit `<Avatar person large>` + `<EmptyState>`. Zero hand-rolled avatars / raw `.empty` left in `shell/`. **Still to audit:** field bricks (`AvatarField`/`ImageField`/`AccentColorField`/`EmojiPicker`). |

...
| 7 | **`EmptyState`** ⏳ (~35 sites left) | the other ~20 templates' `<div className="empty">` | **In progress (2026-06-13)** — `kit/EmptyState.tsx` shipped (wraps existing `.empty` token, backward-compatible; `icon`/`title`/`action` slots + `.empty__icon`/`__title`/`__action`). **Migrated 7 sites: engine `CollectionView` + `DeclarativeApp` (all declarative rooms) + tripsplit, roomledger ×2, groupfund, sitcoop.** `tsc`+`build`+`qa:schema-ui` (0 issues ×3) green. **Remaining ~35 bare `<div className="empty">` across the other templates → migrate across runs** (mechanical: swap tag, add EmptyState to kit import; opportunistically add `action`/`icon` to solo-first first-run empties). |

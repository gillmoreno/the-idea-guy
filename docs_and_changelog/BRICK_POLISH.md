...
| 12 | ~~Audit existing shell bricks against the rubric~~ ✅ DONE | — | ✅ **Complete 2026-06-13.** `ConfirmModal` (focus trap + restore, unique ids). `AddPersonByName` (accessible name + announced error). `ClaimProfile` (dropped a hand-rolled avatar + raw `.empty` → kit `Avatar`/`EmptyState`). `SwitchProfile` clean. **Field bricks** `AccentColorField`/`EmojiPicker`/`ImageField`/`AvatarField` audited — already exemplary (full tablist/listbox ARIA, `useId`, labels); only gap was 2 unlabelled `role="tablist"` (AvatarField's nested photo-source toggle + `AppTabBar`), now `aria-label`led. **Zero unlabelled tablists in the codebase.** All shell + field bricks pass rubric pt 7. |

...
|---|-------|------------------------|---------|
| 1 | **`RecordRow`** ⏳ (coparent left) | coparent record cards (2) | **In progress (2026-06-13)** — `kit/RecordRow.tsx` shipped (slot-based `leading`/`title`/`meta`/`trailing`, real `<button>` + focus ring when `onClick`; `.record-row` tokens). **Migrated 4 cards across 3 templates** (tripsplit expense [clickable], roomledger ledger ×2, groupfund contribution). **Remaining: coparent `MoneyTab`/`MainView` (2 cards) — finish next run.** Meta kept as-is; structured-meta chips are #6 `MetaLine`/#5 `SplitView` slotting into `RecordRow.meta`. `tsc`+`build` green. |

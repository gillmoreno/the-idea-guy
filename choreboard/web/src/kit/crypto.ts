...
/** Symmetric key from arbitrary secret material (family code, parent secret, etc.). */
...
/**
 * Relay room id — unique per app + family + channel (public vs admin).
 * Kids only connect to the `public` room; they never see admin ciphertext.
 */
export async function deriveRoom(
  ...
  const raw = await sha256(
    "lfk-room:" + normalize(appId) + ":" + scope + ":" + normalize(familyCode),
  );
  return toHex(raw.slice(0, 16));
}

/** Admin channel key: requires family code + parent secret (never on kid devices). */
export function adminKeyMaterial(familyCode: string, parentSecret: string): string {
  return `lfk-admin:${normalize(familyCode)}:${normalize(parentSecret)}`;
}

/** Public channel key: family code only. */
export function publicKeyMaterial(familyCode: string): string {
  return `lfk-public:${normalize(familyCode)}`;
}

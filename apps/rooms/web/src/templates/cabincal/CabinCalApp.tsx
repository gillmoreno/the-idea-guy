"use client";

import { useRoomSession } from "@/shell/RoomSessionProvider";
import { RoomConnecting } from "@/shell/RoomConnecting";
import { RoomLoading } from "@/shell/RoomLoading";
import { Setup } from "./components/Setup";
import { ProfilePicker } from "./components/ProfilePicker";
import { MainView } from "./components/MainView";
import { getBuiltinTemplate } from "../registry";
import { useCabinCalStore } from "./lib/useCabinCalStore";

const TEMPLATE = getBuiltinTemplate("cabincal");

export function CabinCalApp() {
  const { mounted, roomCode, hasAdminAccess, isOwner, sync, currentMemberId, version } =
    useRoomSession();
  const store = useCabinCalStore();
  void version;

  if (!mounted)
    return (
      <RoomLoading
        emoji={TEMPLATE?.emoji ?? "🏔️"}
        accent={TEMPLATE?.accent}
        message="Starting Cabin Calendar…"
      />
    );
  if (!roomCode || !store || !sync.localLoaded)
    return (
      <RoomLoading
        emoji={TEMPLATE?.emoji ?? "🏔️"}
        accent={TEMPLATE?.accent}
        message="Loading calendar…"
      />
    );

  if (!store.isInitialized()) {
    if (isOwner && hasAdminAccess) return <Setup />;
    return (
      <RoomConnecting
        emoji="🏔️🗓️"
        title="Connecting to your shared place…"
        organizerLabel="whoever set up the room"
      />
    );
  }

  if (!currentMemberId || !store.getOwner(currentMemberId)) {
    return <ProfilePicker />;
  }

  return <MainView memberId={currentMemberId} />;
}
